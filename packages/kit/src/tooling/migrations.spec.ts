...
import { createMigration, loadApp, writeMigrationSnapshot } from "#tooling/application";
...
describe("Poggers migrations", () => {
  it("hashes structural snapshots and keeps formatting-only edits idempotent", async () => {
    ...
    await writeFile(
      ...
      counterApp(counterTypesV1Formatted, counterDefinitionV1),
      "utf8",
    );
    ...
    await writeFile(
      join(appDir, "src/app.tsx"),
      counterApp(counterTypesV1UiChurn, counterDefinitionV1),
      "utf8",
    );
    ...
    await writeFile(
      join(appDir, "src/app.tsx"),
      counterApp(counterTypesV2, counterDefinitionV2),
      "utf8",
    );
  ...
  it("fails draft and invalid migration edges, then passes reviewed edges", async () => {
    ...
    expect(loaded.api.def.migrationHash).toBe(created.toHash);
...
async function createMigrationFixture(): Promise<string> {
  const appDir = await mkdtemp(join(tmpdir(), "poggers-migrations-"));
  ...
  await symlink(
    resolve(workspaceRoot, "node_modules/@types/bun"),
    join(appDir, "node_modules/@types/bun"),
    "dir",
  );
  await writeFile(
  ...
  await writeFile(
    join(appDir, "src/app.tsx"),
    counterApp(counterTypesV1, counterDefinitionV1),
    "utf8",
  );
  return appDir;
}
...
async function runPoggers(args: string[]): Promise<{ code: number; output: string }> {
  const proc = Bun.spawn(
    ["bun", resolve(workspaceRoot, "packages/kit/src/tooling/cli.ts"), ...args],
    {
      cwd: workspaceRoot,
      stdout: "pipe",
      stderr: "pipe",
...
import type { App as From } from "./snapshots/${fromHash}";
import type { App as To } from "./snapshots/${toHash}";
...
export type App = {
  Resources: {
    counter: {
      ...
      Commands: {
        increment: { Input: { by?: number }; Event: "inc"; Error: never };
      };
    };
  };
  Components: {
...
const counterDefinitionV1 = `export default {
  version: 1,
  resources: {
    counter: {
      state: { count: 0 },
...
const counterDefinitionV2 = `export default {
  version: 2,
  resources: {
    counter: {
...
function counterApp(contract: string, definition: string): string {
  return `import type { AppDef as AppDefinition } from "@poggers/kit";
