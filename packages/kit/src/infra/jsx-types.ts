...
type ContainsFunction<T> = Extract<T, (...args: any[]) => any> extends never ? false : true;

type DOMPropertyValue<T> =
  ContainsFunction<T> extends true ? never : Extract<T, string | number | boolean | null>;

type DOMPropertyProps<T extends Element> = {
  [Key in keyof T as Key extends string
    ? DOMPropertyValue<T[Key]> extends never
      ? never
      : Key
    : never]?: AttributeValue<DOMPropertyValue<T[Key]>>;
};
...
export type CSSProperties = {
  [Property in keyof Omit<
    CSSStyleDeclaration,
    "length" | "parentRule"
  > as CSSStyleDeclaration[Property] extends string ? Property : never]?: CSSPropertyValue;
} & {
...
type GlobalAttributes<T extends Element> = DOMPropertyProps<T> &
  EventHandlerProps<T> &
  DataAttributes &
  AriaAttributes & {
...
> &
  ...
  DOMPropertyProps<HTMLElementTagNameMap[Tag]>;
