...
import { computeSync, serve, type ServerHandle } from "./server";
...
describe("computeSync", () => {
  ...
  function setup(storage?: Store) {
    ...
    return { eventBuffers, states, instanceSeqs, storage: s, adapter: createSingleNodeAdapter(s) };
  }
...
describe("web app shell", () => {
  it("serves PWA metadata, bundled CSS, generated icon, and SPA fallback", async () => {
    ...
    handle = serve(webApp, {
      port: 0,
      adapter: createSingleNodeAdapter(createMemoryStore()),
      web: {
...
describe("buffer trimming", () => {
  ...
  it("computeSync returns snapshot for cursor below trimmed buffer start", () => {
    ...
    const result = computeSync(
      ...
      key,
      trimmed[0]!.seq - 1,
      createSingleNodeAdapter(createMemoryStore()),
      eventBuffers,
      states,
      instanceSeqs,
      app,
