...
import { bundleApp, checkAppConventions, runApp, writeAppTypes } from "./runtime";
...
describe("Poggers app virtual modules", () => {
  it("starts a cold app whose component imports @poggers/app", async () => {
    ...
    const appDir = await mkdtemp(resolve("apps/runtime-style-dev-"));
    ...
    await writeFile(
      ...
      JSON.stringify(
        {
          name: "@poggers/runtime-style-dev-test",
          private: true,
          type: "module",
          dependencies: {
...
`,
  ...
  it("bundles an app that imports direct generated functions from @poggers/app", async () => {
    ...
    const appDir = await mkdtemp(resolve(".app/runtime-style-"));
    createdDirs.push(appDir);
    await mkdir(join(appDir, "src/components"), { recursive: true });
