import { createAnimatable, createLayout, cubicBezier, spring, type Timeline } from "animejs";
...
export function createAnimeLayoutBackend(
  ...
  return {
    create(_key, root, initialChildren) {
      ...
      let animation: Pick<Timeline, "pause"> | undefined;
      let complete: (() => void) | undefined;
      let disposed = false;
      let revision = 0;
      let captured = false;
...
export const browserMotionScheduler: MotionScheduler = {
  now: () => performance.now(),
