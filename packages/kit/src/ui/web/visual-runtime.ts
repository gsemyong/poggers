...
export function visualStyleAttributes(...styles: Array<CompiledVisualStyle | null | undefined>): {
  readonly class?: string;
  readonly style?: Readonly<Record<string, unknown>>;
  readonly "data-style-src"?: string;
} {
  const result = (
    stylexProps as unknown as (...values: readonly unknown[]) => {
...
export type CompiledVisualEntry = {
  readonly style: CompiledVisualStyle | ((...values: unknown[]) => CompiledVisualStyle);
  ...
  const resolvePresentationContract = () => {
    ...
    interactions.forEach((rawInteraction, index) => {
      ...
      const dragOptions: DragOptions = {
        axis:
          configuration.axis === "inline" ||
          configuration.axis === "block" ||
          configuration.axis === "both"
            ? configuration.axis
            : "block",
        bounds: () => {
...
function evaluateVisualExpression(
  ...
  if (
    ...
    expression.operation === "matches" &&
    typeof expression.name === "string"
  ) {
    return context.states?.[expression.name] === true;
  }
...
export function visualConditionMatches(
  ...
  return condition.all.every((predicate) => {
    const matches =
      ...
      (predicate.state == null || context.states?.[predicate.state] === true) &&
