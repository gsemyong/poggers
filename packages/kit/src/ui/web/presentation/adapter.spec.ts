...
import type { WebPresentationLanguage, WebPresentationTokens } from "./language";
...
function webConformanceHarness(): PresentationAdapterConformanceHarness<
  WebPresentationLanguage<TestTheme>,
  Element
> {
  const applications = new WeakMap<Element, number>();
  const releases = new WeakMap<Element, number>();
  const initial = new WeakMap<Element, string>();
  const target = (id: string): FakeElement => {
    const element = createElement(id);
    const base = element.style;
    Object.defineProperty(element, "style", {
      value: new Proxy(base, {
        set(style, property, value) {
          const previous = Reflect.get(style, property);
          const written = Reflect.set(style, property, value);
          if (property === "opacity") {
            applications.set(element, (applications.get(element) ?? 0) + 1);
            if (String(value) === initial.get(element) && previous !== value) {
              releases.set(element, (releases.get(element) ?? 0) + 1);
            }
          }
          return written;
        },
  ...
  return {
    adapter: createWebPresentationAdapter<TestTheme>({
      motionBackend: createMotionBackend([]),
      scheduler: createScheduler(),
    }),
    target,
    declaration: (opacity) => ({ paint: { opacity } }),
    initialize(element, value) {
      const text = String(value);
      initial.set(element, text);
      (element as FakeElement).style.opacity = text;
      applications.set(element, 0);
      releases.set(element, 0);
    },
    inspect: (element) => Number((element as FakeElement).style.opacity),
    applications: (element) => applications.get(element) ?? 0,
    releases: (element) => releases.get(element) ?? 0,
  };
}

presentationAdapterConformance<WebPresentationLanguage<TestTheme>, Element>(
  "web presentation",
