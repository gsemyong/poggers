...
import type { Submission, SubmissionOutcome } from "#kernel/app";
...
export type StatechartTaskScope = StatechartScope & {
...
export type CreateComponentActorOptions = {
  ...
  readonly taskServices?: Readonly<Record<string, unknown>>;
  readonly clock?: StatechartClock;
  readonly refreshSnapshot?: unknown;
  readonly inspect?: (event: StatechartInspectionEvent) => void;
};

export type StatechartInspectionEvent = {
...
export function createComponentActor(options: CreateComponentActorOptions): ComponentActor {
  ...
  const actor = createActor(logic, {
    ...
    ...(options.inspect ? { inspect: options.inspect as never } : {}),
  });
  let started = false;
  let stopped = false;
...
async function runTask(
  task: StatechartTask,
  scope: StatechartTaskScope,
  signal: AbortSignal,
): Promise<unknown> {
  const result = task(scope);
  return isSubmission(result) ? awaitSubmission(result, signal) : await result;
}

function awaitSubmission(
  submission: Submission<unknown>,
  signal: AbortSignal,
): Promise<SubmissionOutcome<unknown>> {
  return new Promise((resolve, reject) => {
    ...
    const finish = (outcome: SubmissionOutcome<unknown>) => {
      if (finished) return;
      finished = true;
      cleanup();
    ...
    unsubscribe = submission.subscribe((current) => {
      if (current.outcome) finish(current.outcome);
    });
    if (finished) unsubscribe();
  });
}

function isSubmission(value: unknown): value is Submission<unknown> {
  if (!value || typeof value !== "object") return false;
  const candidate = value as Partial<Submission<unknown>>;
  return (
...
function absoluteTarget(rootId: string, path: string): string {
  if (!path || path.startsWith("#")) {
    throw new Error(`Poggers targets must be absolute phase paths, received ${path || "empty"}.`);
  }
  return `#${rootId}.${path}`;
}
