...
type RuntimeEvent<Payload = unknown> = {
  id: string;
  seq: number;
  at: number;
...
const handlerApp = defineApp<{
  ...
  Resources: {
    record: {
      ...
      Commands: {
        doRecord: { Input: {}; Event: "recorded"; Error: never };
        addIfPositive: { Input: { amount: number }; Event: "recorded"; Error: never };
      };
    };
  };
}>({
  ...
  resources: {
    record: {
      ...
      commands: {
        ...
        addIfPositive(ctx, { amount: _amount }) {
...
describe("app", () => {
  ...
  describe("snapshot", () => {
    ...
    it("returns deep clone, not reference", () => {
      ...
      state.count = 999;
      expect((snap.data as { count: number }).count).toBe(7);
    });
  ...
  describe("restore", () => {
    ...
    it("returns fresh state for undefined/null snapshot", () => {
      const restored = app.restore(
        "counter",
        null as unknown as { version: number; data: unknown },
      );
      expect(restored).toEqual({ count: 0 });
    });

    it("handles empty snapshot object", () => {
      const restored = app.restore("counter", {} as { version: number; data: unknown });
      expect(restored).toEqual({ count: 0 });
    });

    it("requires an explicit migration path for every mismatched schema hash", () => {
      const hashed = defineApp<{
        Resources: {
          counter: {
            Key: string;
            State: { count: number };
  ...
  describe("runCommand", () => {
    ...
    it("passes current state to command handler", () => {
      ...
      expect(events.length).toBe(1);
      expect((events[0]!.payload as { actorId: string }).actorId).toBe("positive");
    });
    ...
    it("command handler receives zero-state correctly", () => {
      ...
      expect(events.length).toBe(1);
      expect((events[0]!.payload as { actorId: string }).actorId).toBe("zero");
    });
  ...
  describe("command safety", () => {
    it("command cannot mutate real state by modifying ctx.state directly", () => {
      ...
      const testApp = defineApp<{
        ...
        Resources: {
          test: {
            ...
            Views: Record<string, never>;
            Commands: { badMutate: { Input: {}; Event: "mutated"; Error: never } };
          };
        };
      }>({
        version: 1,
        ...
        resources: {
          test: {
            ...
            commands: {
              badMutate(ctx) {
                // @ts-expect-error Command state is deeply read-only.
      ...
      ).toThrow("Resource command state is read-only.");

      expect(realState.count).toBe(100);
    });

    it("createState ignores non-existent resource", () => {
      const state = app.createState("nonexistent");
      expect(state).toBeUndefined();
    });

    it("restore ignores non-existent resource", () => {
      const state = app.restore("nonexistent", {} as { version: number; data: unknown });
      expect(state).toBeUndefined();
    });
  });
