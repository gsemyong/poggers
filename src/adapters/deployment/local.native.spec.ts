...
import type { DependencyBinding } from "@/contracts/deployment";
import type { ProductionArtifacts } from "@/contracts/platform";
import type { System } from "@/core/system";
import { applyDeployment, createDeployment, createRelease, removeDeployment } from "@/deployment";
...
test.skipIf(spawnSync("nats-server", ["--version"], { stdio: "ignore" }).status !== 0)(
  ...
  async () => {
    const directory = await mkdtemp(resolve(tmpdir(), "kit-local-native-deployment-"));
    ...
    if (!program) throw new Error("Native local Deployment fixture has no server Program.");
...
function recorderDependency() {
  return defineServerProductionDependency({
    ...
    crate: {
      ...
      directory: resolve(import.meta.dirname, "../server/production/fixtures/recorder"),
    },
    rust: {
...
export default createSystem({
  metadata: { name: "Local native Deployment" },
