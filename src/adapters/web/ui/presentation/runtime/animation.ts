...
import { eventCursor, readEventOccurrences } from "@/runtime/presentation";
...
export function createWebFrameHost(options: WebFrameHostOptions): WebFrameHost {
  ...
  const request = () => {
    ...
    frame = options.requestFrame((time) => {
      frame = undefined;
      if (disposed) return;
      dispatchTime = finite(time, "Presentation frame time");
      let failure: unknown;
      try {
  ...
  return {
    time() {
      ...
      if (turnTime === undefined) {
        turnTime = finite(options.now(), "Presentation turn time");
        options.queueTurn(() => {
          turnTime = undefined;
        });
      }
    ...
    now() {
      return finite(options.now(), "Presentation time");
    },
    activate(task) {
      if (disposed) return;
      active.add(task);
