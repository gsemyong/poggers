...
import { decay, follow, pulse, spring, track } from "@/platforms/web/presentation/dynamics";
...
describe("web Presentation frame host", () => {
  it("shares one timestamp throughout a synchronous reactive turn", async () => {
    let time = 12;
    const host = createFrameHarness(() => time).host;
  ...
  it("dispatches every active session with one root frame request and timestamp", () => {
    const harness = createFrameHarness(() => 0);
  ...
  it("isolates roots and performs no work after disposal", () => {
    const first = createFrameHarness(() => 0);
    const second = createFrameHarness(() => 0);
    const task = vi.fn();
...
function createFrameHarness(now: () => number) {
  let requests = 0;
  let cancels = 0;
