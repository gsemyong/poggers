...
import { activateJSXRenderer, jsx } from "@/jsx/runtime";
...
describe("shared JSX protocol", () => {
  ...
  test("temporarily advances one Platform renderer during transactional replacement", () => {
    const firstRenderer = vi.fn(() => "first");
    const secondRenderer = vi.fn(() => "second");
    const first = activateJSXRenderer(firstRenderer, "web");
    const second = activateJSXRenderer(secondRenderer, "web");

    expect(jsx("surface", {})).toBe("second");
    second[Symbol.dispose]();
    expect(jsx("surface", {})).toBe("first");

    expect(() => activateJSXRenderer(secondRenderer, "native")).toThrow(
      "A different UI Platform JSX renderer is already active",
    );
    first[Symbol.dispose]();
  });
});
