...
/**
 * Declares the statically named native methods that generated Dependency
 * adapters require. The generated Program, rather than a hand-written
 * registry, chooses and verifies the complete operation set.
 */
#[macro_export]
macro_rules! dependency_operations {
    ($implementation:ty { $($method:ident => $operation:literal),+ $(,)? }) => {
        impl $implementation {
            $(
                pub fn $method(
                    &self,
                    engine: $crate::Engine,
                    input: $crate::Value,
                    invocation: $crate::DependencyInvocation,
                ) -> $crate::NativeFuture<$crate::Value> {
                    <$implementation as $crate::Dependency>::call(
                        self,
                        engine,
                        $operation,
                        input,
