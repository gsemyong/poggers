  ...
  generate(): string {
    ...
    const starts = this.#linked.contributions.flatMap(({ contribution }) => {
      ...
      const requirements = contribution.requires
        .map(
          ({ name }) =>
            `(${rustString(name)}.to_owned(), engine.dependency_value(${rustString(name)})?),`,
        )
        .join("\n        ");
    ...
    let result = ${functionName}(engine.clone(), Vec::new(), Vec::new(), Some(dependencies)).await?;
    ${retain}`,
      ];
    });
...
`;
  ...
  #export(definition: RustProgramFunctionExport): string {
    if (!/^[a-z][a-z0-9_]*$/.test(definition.name)) {
      throw new Error(`Invalid production Program export ${JSON.stringify(definition.name)}.`);
    }
    const functionName = this.#functionIds.get(
      `${definition.contribution}\0${definition.function}`,
    );
    if (!functionName) {
      throw new Error(
        `Production Program export ${JSON.stringify(definition.name)} references missing function ` +
          `${JSON.stringify(`${definition.contribution}:${definition.function}`)}.`,
      );
    }
    const dependencies = definition.dependencies
      ...
      .join("\n        ");
    return `pub fn ${definition.name}(engine: Engine, input: Value) -> NativeFuture<Value> {
    Box::pin(async move {
        let mut context = input.as_record()?.as_ref().clone();
        let dependencies = Value::record(BTreeMap::from([
        ${dependencies}
        ]));
        context.insert("dependencies".to_owned(), dependencies);
        ${functionName}(engine, Vec::new(), vec![Value::record(context)], None).await
    })
}`;
  }
  ...
  #function(contribution: string, function_: FunctionIR, start: boolean): string {
    ...
    if (start) {
      setup.push(
        `let ${binding("dependencies")} = cell(dependencies.ok_or_else(|| ` +
          `NativeError::new("MissingDependencyScope", "Program start has no Dependency scope."))?);`,
      );
    }
...
fn ${name}(
    ...
    Box::pin(async move {
        let _ = (&engine, &captures, &arguments, &dependencies);
        ${setup.join("\n        ")}
  ...
  .join("\n")}
      ...
      case "dependency-reference":
        return `Value::mutable_record([
            ...
            ("__kit_binding".to_owned(), ${this.#expression(contribution, expression.binding)}),
        ])`;
