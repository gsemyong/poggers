...
export async function buildRustApplication(options: {
  ...
  return emitRustProgram({
    ...
    adapterSource: options.adapter
      ? await readFile(resolve(directory, options.adapter), "utf8")
      : undefined,
  });
}

function applicationEntry(directory: string): string {
  for (const name of ["app.tsx", "app.ts"]) {
    const application = resolve(directory, "src", name);
    try {
      if (statSync(application).size > 0) return application;
    } catch {
      continue;
    }
  }
