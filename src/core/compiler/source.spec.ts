...
import { executeProgramFixtureIR, executeProgramIR } from "@/core/development";
...
describe("Poggers Application compiler", () => {
  ...
  test("distinguishes synchronous and asynchronous Capability operations", async () => {
    const source = applicationSource()
      .replace(
        "read(input: { count: number }): Promise<readonly number[]>;",
        "read(input: { count: number }): Promise<readonly number[]>;\n  offset(input: {}): number;",
      )
      .replace("let total = 0;", "let total = capabilities.numbers.offset({});");
    ...
    const unawaited = await fixture(
      applicationSource().replace(
        "const values = await capabilities.numbers.read({ count: 4 });",
        "capabilities.numbers.read({ count: 4 });\n        const values: readonly number[] = [];",
      ),
    );
  ...
  test("executes the extracted process through injected Capabilities", async () => {
    ...
    expect(execution.calls).toEqual([
      { capability: "numbers", operation: "read", input: { count: 4 } },
      {
        capability: "output",
        operation: "write",
...
`,
  ...
  test("generates and runs a standalone native artifact from the same portable IR", async () => {
    const ir = compileApplication(await fixture(applicationSource()));
    const program = programContribution(ir, "feature/worker/program/cloud")!;
    const directory = await temporaryDirectory("poggers-native-");
...
function createTasksFeature<const Name extends string>(name: Name, implementation: {
  run(input: {
    capabilities: { repository: { read(input: {}): Promise<readonly string[]> } };
  }): Promise<void>;
}): Feature<Tasks> {
  return {
    programs: {
...
const tasks = createTasksFeature("tasks", {
  async run({ capabilities }) {
    const tasks = await capabilities.repository.read({});
...
function defineServerFeature<Contract>(threshold: number): Feature<Tasks> {
  const server = {
    async start({ capabilities }: { capabilities: { repository: Repository } }) {
      const values = await capabilities.repository.read({});
