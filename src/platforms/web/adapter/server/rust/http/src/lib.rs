...
async fn web_artifact_response(
    ...
    if path == "/robots.txt" || path == "/sitemap.xml" {
        let (body, content_type) = if path == "/robots.txt" {
            (&web.discovery.robots, "text/plain; charset=utf-8")
        } else {
            (&web.discovery.sitemap, "application/xml; charset=utf-8")
        };
        return Response::builder()
    ...
    let (status, document, etag, markdown_document, markdown_etag, cache_control) = match web
        .document
        .lookup(path, request.uri().query())
    {
        RouteLookup::Found {
            ...
            markdown_etag,
            cache_control,
        } => (status, html, etag, markdown, markdown_etag, cache_control),
    ...
    if representation == WebRepresentation::RouteData {
    ...
    if status == 200 && request_matches(&request, etag) {
...
async fn render_dynamic_web(
    ...
    let mut outcomes = Vec::with_capacity(render.branch().len());
    let mut deferred_tasks = Vec::new();
    let mut deferred_offset = 0;
    for entry in render.branch() {
        if !entry.loader {
            outcomes.push(None);
            continue;
        }
        let (engine, handle) = loader.as_ref().expect("loader required by branch");
        let mut input = serde_json::Map::from_iter([
            (
                "route".to_owned(),
                serde_json::Value::String(entry.route.clone()),
            ),
            ("params".to_owned(), entry.params.clone()),
            ("search".to_owned(), entry.search.clone()),
        ]);
        if !entry.shared {
        ...
        let (value, tasks) = prepare_web_loader_outcome(&entry.deferred, deferred_offset, value)
            .map_err(|error| format!("web Route loader output failed: {error}"))?;
        deferred_offset += entry.deferred.len();
        outcomes.push(Some(value));
        deferred_tasks.extend(tasks);
    }
    let deferred_engine = loader.map(|(engine, _)| engine);
    let outcome = web
        .document
        .render_request(render, outcomes)
...
fn cacheable_web_outcome(rendered: &RenderedWebOutcome) -> Option<CachedWebOutcome> {
    ...
    match &rendered.outcome {
        WebLoaderOutcome::Document {
            ...
            cache_control,
            deferred: None,
        } if *status < 500 => Some(CachedWebOutcome::Document {
            status: *status,
            html: html.clone(),
            etag: etag.clone(),
        ...
        WebLoaderOutcome::Document { .. } | WebLoaderOutcome::Redirect { .. } => None,
    }
}
...
fn dynamic_web_response(
    ...
    match rendered.outcome {
        WebLoaderOutcome::Document {
            ...
            if (route_data || status == 200) && request_matches(request, &etag) {
                let mut response = not_modified(&etag, &cache_control);
                response
                    .headers_mut()
            ...
            Response::builder()
                .status(if route_data { 200 } else { status })
                .header(header::CONTENT_TYPE, content_type)
                .header(header::CONTENT_LENGTH, body.len().to_string())
                .header(header::CACHE_CONTROL, cache_control)
                .header(header::ETAG, etag)
        ...
        WebLoaderOutcome::Redirect { location, status } => {
            if route_data {
...
fn prepare_web_loader_outcome(
    ...
    if has_redirect || deferred.is_empty() {
        return value
            .to_json()
    ...
    for (name, item) in data.iter() {
        let Some(index) = deferred.iter().position(|field| field == name) else {
            json_data.insert(
                name.clone(),
        ...
        let boundary = format!("d{}", offset + index);
        json_data.insert(
            name.clone(),
            serde_json::json!({
                "version": 1,
    ...
    if let Some(status) = outcome.get("status") {
        json_outcome.insert(
            "status".to_owned(),
            status.to_json().map_err(|error| error.to_string())?,
        );
    }
